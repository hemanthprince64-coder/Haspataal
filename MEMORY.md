...
- **Phase 1 Complete**: Implemented Phase 1 Canonical Patient Registry following the Identity Constitution. Added packages/core/domain/identity utilities (cryptography, normalization, service layer, adapters, events). Generated and ran Prisma schema migrations. Note: Local testing required bypassing the monorepo test runner due to global package config issues, and prisma db execute on Supabase was blocked via network restrictions, but all tasks are successfully implemented in code.
-   * * T e s t c o n t a i n e r s   P o s t g r e S Q L   I d e m p o t e n c y : * *   W h e n   r u n n i n g   i n t e g r a t i o n   t e s t s   w i t h   T e s t c o n t a i n e r s ,   e n s u r e   o u t b o x   r e l a y   w o r k e r s   u s e   r a w   S Q L   ( \ \ \ )   w i t h   \ F O R   U P D A T E   S K I P   L O C K E D \   e x e c u t e d   f u l l y   i n s i d e   a   s i n g l e   t r a n s a c t i o n   b o u n d a r y   t o   g u a r a n t e e   i d e m p o t e n c y   a n d   p r e v e n t   c o n c u r r e n t   d o u b l e - p r o c e s s i n g   o f   e v e n t s .  
 
- **Phase 4 Complete**: Established durable projection architecture. Timeline, Journey, Analytics, Search, Bed Management, and Notification consumers are replay-safe, idempotent, append-only, and fully derived from canonical Outbox events. Projection state is disposable and recoverable through deterministic replay, preserving the event-driven architecture. Future operational hardening should include Projection Drift Detection, Replay Locks, Projection Metrics, and Consumer Version Rollouts.
- **Phase 5A Complete**: Implemented Canonical Clinical Orders Engine. Separated static catalog definition (`ClinicalOrderCatalog`) from order instances. Separated clinical intent (`Order`) from fulfillment (`Execution`). All order mutations (create, amend, cancel, group complete) leverage Prisma `$transaction` emitting versioned outbox events (`ORDER_REQUESTED`, `ORDER_AMENDED`, `ORDER_CANCELLED`, `ORDER_COMPLETED`). Enforced Phase 2 Authorization Engine rules for all canonical operations. State mutations correctly use immutable `OrderVersion` records and update base `Order.activeVersionId` efficiently. Tests run via `vitest` covering concurrent modifications and DB constraints.
- **ESLint in Tests:** When writing tests (especially with Vitest/Prisma mocks), use `@ts-expect-error` instead of `@ts-ignore` to bypass TypeScript compiler checks, as `@typescript-eslint/ban-ts-comment` is actively enforced and will fail pre-commit hooks. _(Added 2026-07-13)_
- **Phase 5B.1 Complete**: Implemented Pharmacy Execution Engine. Separated PharmacyExecution from Canonical Order. Introduced PharmacyDispense (renamed to PharmacyExecutionDispense to avoid legacy collisions). Handled stock reservations with FEFO batch selection. Used $transaction and outbox for events (MEDICATION_DISPENSED, MEDICATION_ADMINISTERED). Integrated PharmacyConsumer into the platform-contracts event framework. Implemented 15 integration tests proving rollback, isolation, and core workflows. _(Added 2026-07-13)_
- **Prisma Concurrency Locks**: When implementing concurrency control in Prisma (like tracking dispensed quantities), indUnique inside a $transaction does NOT take an exclusive row lock in PostgreSQL. To prevent race conditions, explicitly lock the record using wait tx.`SELECT id FROM <table_name> WHERE id = \ FOR UPDATE` before reading or modifying the data. _(Added 2026-07-13)_

- **Phase 5B.2 Complete**: Implemented Laboratory Execution Engine. Separated Laboratory Execution from Canonical Order. Introduced LaboratoryResultVersion for immutable result tracking. Managed specimen collection and rejection lifecycle. Handled critical values and analyzer queues. Injected PrismaClient into domain services rather than using global imports to prevent connection URL initialization errors in testing environments. Used raw `SELECT FOR UPDATE` locks during concurrent result verification to prevent version collision. Integrated LaboratoryConsumer into platform-contracts. Implemented 9 integration tests proving isolation, concurrency safety, and transactionality. _(Added 2026-07-13)_
