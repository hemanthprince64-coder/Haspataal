      ...
      <body className={`${inter.className} bg-slate-950 text-white min-h-screen flex`}>
        ...
        <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col p-6 shrink-0">
          ...
          <nav className="flex flex-col gap-2 flex-1">
            <Link
              href="/"
              className="flex items-center gap-3 px-4 py-2 rounded-lg bg-emerald-500/10 text-emerald-400 font-medium text-sm"
            >
              <span>🏥</span> HMS Overview
            </Link>
            <Link
              href="/doctors"
              className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/5 text-slate-400 hover:text-white text-sm transition-colors"
            >
              <span>👨‍⚕️</span> Doctors
            </Link>
            <Link
