...
async function runConcurrent(label, url, count) {
  ...
  const promises = Array.from({ length: count }, (_, i) => timedFetch(`${label}#${i + 1}`, url));
...
async function main() {
  ...
  for (const r of results) {
    ...
    console.log(
      `  ${name} | ${String(r.ok).padEnd(4)} | ${String(r.fail).padEnd(4)} | ${String(r.avg + 'ms').padEnd(5)} | ${String(r.p95 + 'ms').padEnd(5)} | ${r.max}ms`,
    );
  }
