...
async function runSecurityTests() {
  ...
  const patientToken = await generateMockToken({
    user: { id: 'p1', role: 'PATIENT', name: 'Frank' },
  });
  // Assuming patient auth cookie maps to session_admin somehow by attacker renaming cookie name
  await test(
    'Patient Role accessing Admin',
    `${baseUrl}/admin/dashboard`,
    `session_admin=${patientToken}`,
    '/admin',
  );
  ...
  const hospToken = await generateMockToken({
    user: { id: 'h1', role: 'HOSPITAL_ADMIN', name: 'Hosp', hospitalId: realHospitalId },
  });
  await test(
    'Hospital Admin accessing Platform Admin',
    `${baseUrl}/admin/dashboard`,
    `session_admin=${hospToken}`,
    '/admin',
  );

  // 4. Patient token to Hospital Dashboard
  await test(
    'Patient accessing Hospital Dashboard',
    `${baseUrl}/hospital/dashboard`,
    `session_user=${patientToken}`,
    '/hospital/login',
  );
