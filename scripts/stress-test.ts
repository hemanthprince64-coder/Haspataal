...
async function runConcurrencyTest() {
  ...
  try {
    ...
    logger.info(
      { doctorId, hospitalId, date: dateString, slot: slotTime },
      'Generated test slot targets',
    );
    ...
    if (successes.length === 1 && failures.length === 4) {
      ...
      logger.fatal(
        '❌ STRESS TEST FAILED: Concurrency leak detected! Bookings: ' + successes.length,
      );
      process.exit(1);
    }
    ...
    if (dbVerification === 1) {
      ...
      logger.fatal(
        '❌ DB INTEGRITY FAILED: Duplicate rows found in database! Rows: ' + dbVerification,
      );
      process.exit(1);
    }
  } catch (e) {
