...
async function runSecurityTests() {
  ...
  let { data: patients, error: readRoleError } = await supabase
  ...
  let { error: insertError } = await supabase
    .from('appointments')
    .insert([{ patient_id: 'fake-id', doctor_id: 'fake-id' }]);
  if (insertError) {
  ...
  if (testsFailed === 0) {
    console.log(
      '✅ SYSTEM IS SECURE! RLS is active and protecting against unauthenticated access.',
    );
  } else {
