...
export const services = {
  ...
  platform: {
    ...
    searchDoctors: (city, speciality, query) => {
      ...
      let doctors = db.users.filter(
        (u) => u.role === 'DOCTOR' && hospitalIds.includes(u.hospitalId),
      );
      if (speciality) {
    ...
    getDoctorCount: (city) => {
      const hospitalIds = db.hospitals
        .filter((h) => h.city.toLowerCase() === city.toLowerCase() && h.status === 'ACTIVE')
        .map((h) => h.id);
      return db.users.filter((u) => u.role === 'DOCTOR' && hospitalIds.includes(u.hospitalId))
        .length;
    },
    ...
    getAllSpecialities: () => {
      return [
        ...new Set(
          db.users.filter((u) => u.role === 'DOCTOR' && u.speciality).map((u) => u.speciality),
        ),
      ];
    },
  ...
  hospital: {
    ...
    removeDoctor: (hospitalId, doctorId) => {
      const idx = db.users.findIndex(
        (u) => u.id === doctorId && u.hospitalId === hospitalId && u.role === 'DOCTOR',
      );
      if (idx !== -1) {
    ...
    createVisit: (hospitalId, data) => {
      let hp = db.hospitalPatients.find(
        (p) => p.hospitalId === hospitalId && p.mobile === data.patientMobile,
      );
  ...
  admin: {
    ...
    getPlatformStats: () => {
      return {
        ...
        cities: [...new Set(db.hospitals.filter((h) => h.status === 'ACTIVE').map((h) => h.city))]
          .length,
      };
    },
  },
