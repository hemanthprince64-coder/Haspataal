...
export default async function DoctorDashboard() {
  ...
  const upcomingAppointments = appointments.filter(
    (a) => new Date(a.date) > today && !isToday(a.date),
  );
    ...
    <div className="page-enter">
      ...
      <div className="grid-layout">
        ...
        >
          ...
          <p style={{ opacity: 0.9 }}>
            You have <strong style={{ color: '#38bdf8' }}>{todaysAppointments.length}</strong>{' '}
            appointments today.
          </p>
        </div>
              ...
              {todaysAppointments.map((app) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <Link
                      href={`/dashboard/doctor/record/${app.patientId}`}
                    ...
                    {app.status !== 'PENDING' && (
                      <span
                        className={`badge ${app.status === 'COMPLETED' ? 'badge-success' : 'badge-danger'}`}
                      >
                        {app.status}
                      </span>
                    )}
