...
export function Step6Review({ config, onActivate }: { config: any; onActivate: () => void }) {
  const totalBeds = Object.values(config.bed_counts || {}).reduce<number>(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-green-50 border border-green-200 text-green-800 p-4 rounded-lg flex items-start gap-3 mt-6">
        ...
        <div>
          ...
          <p className="text-sm mt-1">
            Clicking "Activate Hospital" will finalize your setup and open your live HMS dashboard.
            You can edit these settings later from the Admin panel.
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-end pt-6">
        <Button
          onClick={onActivate}
          className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded shadow-lg transition-transform hover:scale-105"
        >
          Activate Hospital
        </Button>
      </div>
