          ...
          <CardContent className="p-4 space-y-4">
            <div>
              ...
              <p className="text-sm font-medium mt-1">
                High fever, severe headache, body ache x 3 days.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-sm bg-blue-50 p-3 rounded-lg border border-blue-100">
              <div>
                <span className="text-slate-500">Temp:</span> 102.4°F
              </div>
              <div>
                <span className="text-slate-500">BP:</span> 120/80
              </div>
              <div>
                <span className="text-slate-500">SpO2:</span> 98%
              </div>
              <div>
                <span className="text-slate-500">Pulse:</span> 110
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                ...
                <span className="bg-slate-100 text-slate-700 text-xs px-2 py-1 rounded border border-slate-200">
                  Penicillin Allergy
                </span>
              </div>
            </div>
          ...
          <CardContent className="p-6 flex-1 overflow-y-auto space-y-6">
              ...
              <div className="space-y-2 mb-3">
                {medicines.map((med, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-slate-50 p-2 rounded border">
                    ...
                    <span className="text-sm text-slate-600 w-1/4">
                      {med.frequency} x {med.duration}
                    </span>
                    <button
                      className="text-red-500 hover:text-red-700 text-sm ml-auto"
                      onClick={() => setMedicines((m) => m.filter((_, i) => i !== idx))}
                    >
                      Remove
                    </button>
                  </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                <span
                  className="text-xs bg-slate-100 hover:bg-slate-200 cursor-pointer px-2 py-1 rounded"
                  onClick={() => setAdvice((a) => a + ' Review after 5 days.')}
                >
                  + Review 5 days
                </span>
                <span
                  className="text-xs bg-slate-100 hover:bg-slate-200 cursor-pointer px-2 py-1 rounded"
                  onClick={() => setAdvice((a) => a + ' Drink warm water.')}
                >
                  + Warm water
                </span>
              </div>
