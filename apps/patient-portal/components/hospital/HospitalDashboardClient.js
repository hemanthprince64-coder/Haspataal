...
const EVENT_COLORS = {
  ...
  followup_created: '#14b8a6',
  bill_paid: '#22c55e',
  notification_sent: '#818cf8',
  patient_admitted: '#ef4444',
  doctor_prescribes: '#0ea5e9',
  patient_registered: '#3b82f6',
  patient_visited: '#14b8a6',
};
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1rem' }}>
        <svg width="100" height="100" viewBox="0 0 100 100">
          ...
          />
          <text x="50" y="55" textAnchor="middle" fontSize="18" fontWeight="800" fill="#0f172a">
            {pct}%
          </text>
        </svg>
        <div>
          <div style={{ fontSize: '0.72rem', color: '#6b7280', marginBottom: '0.35rem' }}>
            Discharged → Follow-up (30d)
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <div
                style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#14b8a6' }}
              />
              <span style={{ fontSize: '0.72rem', fontWeight: '600' }}>Your hospital {pct}%</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <div
                style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#e5e7eb' }}
              />
              <span style={{ fontSize: '0.72rem', color: '#9ca3af' }}>
                Network avg {kpi?.networkAvg ?? 63}%
              </span>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <span
                style={{
                  fontSize: '0.72rem',
                  fontWeight: '700',
                  color: PATHWAY_COLORS[p.key] || '#6b7280',
                }}
              >
                {p.completionPct}%
              </span>
            ...
            <div style={{ height: '5px', background: '#f1f5f9', borderRadius: '999px' }}>
              <div
                style={{
                  ...
                  background: PATHWAY_COLORS[p.key] || '#6b7280',
                  borderRadius: '999px',
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {events.map((ev) => (
          <div
            ...
            style={{
              ...
              borderLeft: `2px solid ${EVENT_COLORS[ev.eventType] || '#94a3b8'}`,
              paddingLeft: '0.75rem',
              marginLeft: '-1.25rem',
            }}
          >
            ...
            <div>
              <div
                style={{
                  ...
                  fontSize: '0.7rem',
                  fontWeight: '600',
                  color: EVENT_COLORS[ev.eventType] || '#6b7280',
                }}
              >
                {ev.eventType}
              </div>
        ...
        {events.length === 0 && (
          <div
            style={{ color: '#9ca3af', fontSize: '0.8rem', textAlign: 'center', padding: '1.5rem' }}
          >
            No events yet today
          </div>
        )}
    ...
    >
      ...
      >
        ...
        {(pending.followups?.urgent ?? 0) > 0 && (
          ...
          >
            {pending.followups.urgent} urgent
          </span>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.25rem' }}>
          {[
            ['pending', 'Due Soon'],
            ['missed', `Missed (${missed.followups?.total ?? 0})`],
          ].map(([k, l]) => (
            <button
              key={k}
              onClick={() => setTab(k)}
              style={{
                ...
                borderRadius: '6px',
                border: '1px solid',
                borderColor: tab === k ? '#14b8a6' : '#e5e7eb',
                background: tab === k ? '#f0fdfa' : 'transparent',
                color: tab === k ? '#0d9488' : '#6b7280',
                cursor: 'pointer',
              }}
            >
              {l}
            ...
            <tbody>
                ...
                <tr key={f.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                  <td style={{ padding: '0.55rem 0.5rem' }}>
                    <div
                      style={{
                        ...
                        height: '8px',
                        borderRadius: '50%',
                        background: URGENCY_DOT[f.urgency] || '#9ca3af',
                      }}
                    />
                  </td>
                  <td style={{ padding: '0.55rem 0.5rem' }}>
                  ...
                  <td style={{ padding: '0.55rem 0.5rem' }}>
                    <span
                      style={{
                        fontSize: '0.68rem',
                        background: (PATHWAY_COLORS[f.carePathway] || '#3b82f6') + '18',
                        color: PATHWAY_COLORS[f.carePathway] || '#3b82f6',
                        fontWeight: '700',
                  ...
                  <td
                    style={{
                      ...
                      color: f.urgency === 'urgent' ? '#dc2626' : '#374151',
                      fontWeight: f.urgency === 'urgent' ? '700' : '500',
                    }}
                  >
                    {f.dueDateLabel}
                  </td>
                  <td style={{ padding: '0.55rem 0.5rem' }}>
                    <button
                      onClick={() => {
                        fetch(`/api/hospitals/${hospitalId}/followups/${f.id}/remind`, {
                          method: 'POST',
              ...
              {rows.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    style={{ textAlign: 'center', color: '#9ca3af', padding: '1.5rem' }}
                  >
  ...
  const channels = [
    {
      ch: 'WhatsApp',
      icon: '💬',
      sent: data?.whatsapp?.sent ?? 0,
      delivered: data?.whatsapp?.delivered ?? 0,
      color: '#22c55e',
    },
    {
      ch: 'SMS Fallback',
      ...
      delivered: data?.sms?.delivered ?? 0,
      color: '#3b82f6',
    },
  ];
  return (
    ...
    >
      <div
        style={{ fontWeight: '700', fontSize: '0.9rem', color: '#0f172a', marginBottom: '0.75rem' }}
      >
        Today&apos;s Sends
      </div>
      <div
          ...
          >
            ...
            >
              <div
                style={{
                  height: '4px',
                  width: `${c.sent ? Math.round((c.delivered / c.sent) * 100) : 0}%`,
                  background: c.color,
                  borderRadius: '999px',
                }}
              />
...
function RevenueCard({ data, isLoading, isError, mutate }) {
  ...
  return (
    ...
    >
      ...
      >
        {formatINR(data?.totalRecovered)}
      </div>
      <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '1.25rem' }}>
        this {data?.period || 'month'} via retention engine
      </div>
              ...
              {r.growthPct !== 0 && (
                <span
                  style={{
                    fontSize: '0.68rem',
                    fontWeight: '700',
                    color: r.growthPct > 0 ? '#22c55e' : '#ef4444',
                    background: r.growthPct > 0 ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)',
    ...
    [
      {
        ...
        value: m?.todaysVisits?.value ?? '—',
        icon: '🗓',
        color: '#14b8a6',
        sub: 'OPD + IPD combined',
        delta: m?.todaysVisits?.deltaLabel || null,
      },
      {
        label: 'Total Patients',
        value: m?.totalPatients?.value ?? '—',
        icon: '👥',
        color: '#3b82f6',
        sub: `${m?.totalPatients?.newToday ?? 0} new today`,
        delta: null,
      },
      {
        label: 'Total Doctors',
        value: m?.totalDoctors?.value ?? '—',
        icon: '👨‍⚕️',
        color: '#8b5cf6',
        sub: `${m?.totalDoctors?.onDutyToday ?? 0} on duty`,
        delta: null,
      },
    ],
    [
      {
        label: 'Scheduled',
        value: m?.scheduled?.value ?? '—',
        icon: '⏰',
        color: '#f59e0b',
        sub: `${m?.scheduled?.pending ?? 0} pending`,
        delta: null,
      },
      {
        label: 'Bed Occupancy',
        value: m ? `${m.bedOccupancy?.pct ?? 0}%` : '—',
        icon: '🛏',
        color: '#ef4444',
        sub: m ? `${m.bedOccupancy.occupied}/${m.bedOccupancy.total} beds` : '',
        delta: null,
      },
      {
        label: 'Completed',
        ...
        icon: '✅',
        color: '#22c55e',
        sub: `${m?.completed?.completionRate ?? 0}% rate`,
        delta: null,
      },
    ],
  ];
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ color: '#9ca3af', fontSize: '0.85rem', margin: '0.25rem 0 0' }}>
            Welcome back, <strong style={{ color: '#374151' }}>{user.name}</strong> ·{' '}
            {initialHospital?.legalName || initialHospital?.name || 'Your Hospital'}
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
        {[
          { href: '/hospital/dashboard/reports', label: '📊 Reports' },
          { href: '/hospital/dashboard/doctors', label: '👨‍⚕️ Doctors' },
