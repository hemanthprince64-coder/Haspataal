    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      {prescriptions.map((pres) => {
        ...
        return (
          <Card
            key={pres.id}
            className="group hover:border-blue-300 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 rounded-[2.5rem] bg-white border-slate-200/60 overflow-hidden relative"
          >
            <div
              className={`absolute top-0 left-0 w-2 h-full ${isFileUpload ? 'bg-amber-400' : 'bg-blue-500'}`}
            />
            ...
            <CardContent className="p-8">
              <div className="flex justify-between items-start gap-4 mb-8">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all duration-500 group-hover:scale-110 shadow-sm ${isFileUpload ? 'bg-amber-50 border-amber-100 text-amber-600' : 'bg-blue-50 border-blue-100 text-blue-600'}`}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge
                        variant="outline"
                        className={`text-[9px] font-black uppercase tracking-widest px-2 py-0 border-none ${isFileUpload ? 'bg-amber-100/50 text-amber-700' : 'bg-blue-100/50 text-blue-700'}`}
                      >
                        {pres.type}
                      </Badge>
              ...
              {isFileUpload && pres.fileUrl ? (
                <Button
                  asChild
                  className="w-full h-16 bg-amber-500 hover:bg-amber-600 text-white rounded-[1.25rem] font-black text-base shadow-xl shadow-amber-500/20 transition-all hover:-translate-y-1 active:scale-95"
                >
                  <a
                    href={pres.fileUrl}
                    target="_blank"
                  ...
                  >
                    <Eye className="w-5 h-5" /> View Scanned Document{' '}
