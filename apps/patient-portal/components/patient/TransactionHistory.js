    ...
    return (
      <div className="py-24 text-center bg-slate-50/30">
        ...
        <p className="text-slate-500 text-lg font-medium max-w-sm mx-auto leading-relaxed">
          Once you start using your wallet for bookings and services, your activity will appear
          here.
        </p>
      </div>
    );
    ...
    <div className="divide-y divide-slate-100">
      {transactions.map((txn, index) => {
        ...
        return (
          <div
            key={txn.id}
            className="flex justify-between items-center p-6 hover:bg-slate-50 transition-all group duration-300"
          >
            <div className="flex items-center gap-5">
              <div
                className={`relative w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${isCredit ? 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100' : 'bg-red-50 text-red-500 group-hover:bg-red-100'}`}
              >
                <Icon className="w-7 h-7" />
                <div
                  className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-lg border-2 border-white flex items-center justify-center shadow-md ${isCredit ? 'bg-emerald-500' : 'bg-rose-500'}`}
                >
                  {isCredit ? (
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge
                    variant="outline"
                    className={`text-[9px] font-black uppercase tracking-widest px-1.5 py-0 border-none bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-400`}
                  >
            ...
            <div className="text-right">
              <div
                className={`text-2xl font-black tabular-nums tracking-tighter ${isCredit ? 'text-emerald-600' : 'text-slate-900'}`}
              >
                {isCredit ? '+' : '-'}₹{amntStr}
              </div>
