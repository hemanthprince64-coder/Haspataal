  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {stats.map(({ label, value, unit, Icon, color, bgColor }) => (
        <Card
          key={label}
          className={`rounded-[2rem] border-none shadow-xl shadow-slate-200/20 group hover:scale-[1.05] transition-all duration-500 overflow-hidden`}
        >
          <CardContent
            className={`p-6 flex flex-col items-center justify-center text-center h-full relative ${bgColor}`}
          >
            <div className="absolute top-0 right-0 p-2 opacity-5 group-hover:opacity-10 transition-opacity">
              <Icon className="w-12 h-12" />
            </div>
            <div
              className={`w-12 h-12 rounded-xl bg-white flex items-center justify-center mb-4 shadow-sm group-hover:shadow-md transition-all group-hover:rotate-6 ${color}`}
            >
            ...
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest opacity-80">
              {label}{' '}
              {unit && <span className="text-[8px] font-bold opacity-50 ml-0.5">({unit})</span>}
            </div>
          </CardContent>
        </Card>
      ))}
