...
export default function AppointmentsList({ appointments, tab, now, onReviewSubmitted }) {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      setReviewOpen(false);
      if (onReviewSubmitted) onReviewSubmitted(reviewingId);
    } catch (e) {
    ...
    return (
      <div className="py-24 text-center bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200/60 animate-in fade-in zoom-in duration-500">
        ...
        <Button
          asChild
          variant="outline"
          className="mt-10 border-slate-200 hover:bg-white rounded-2xl h-14 px-8 font-black text-slate-600 transition-all hover:scale-105 active:scale-95"
        >
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {appointments.map((visit) => {
          ...
          return (
            <Card
              key={visit.id}
              className="group hover:border-blue-400 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 rounded-[2rem] bg-white border-slate-200/60 overflow-hidden"
            >
              <CardContent className="p-0">
                <div className="p-6 sm:p-8">
                  <div className="flex justify-between items-start gap-4 mb-6">
                    ...
                    <Badge
                      variant="outline"
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase border transition-colors ${statusColor}`}
                    >
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-slate-100">
                    ...
                    <div className="p-2 bg-slate-50 rounded-xl group-hover:bg-white transition-colors">
                      <div className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-0.5">
                        Booked On
                      </div>
                      <div className="text-[10px] font-bold text-slate-600 italic">
                        {visit.createdAt
                          ? format(new Date(visit.createdAt), 'dd MMM, hh:mma')
                          : 'N/A'}
                      </div>
                    </div>
                  </div>
                ...
                {tab === 'upcoming' && (
                  <div className="flex gap-1.5 p-3 bg-slate-50 border-t border-slate-100 group-hover:bg-blue-50/30 transition-colors">
                    ...
                    <Button
                      asChild
                      variant="outline"
                      className="flex-1 h-12 bg-white border-slate-200 hover:border-blue-400 hover:text-blue-600 rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-sm"
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-12 w-12 rounded-xl text-slate-300 hover:text-slate-600"
                    >
                      <MoreVertical className="w-5 h-5" />
                    </Button>
                  </div>
                )}
                {tab !== 'upcoming' && visit.status === 'COMPLETED' && (
                  <div className="flex gap-1.5 p-3 bg-slate-50 border-t border-slate-100 group-hover:bg-blue-50/30 transition-colors">
                    <Button
                      onClick={() => openReview(visit)}
                      className="flex-1 h-12 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-sm"
                    >
                      <Star className="w-4 h-4 mr-2 fill-white" /> Rate Visit
                    </Button>
                  </div>
                )}
              </CardContent>
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="sm:max-w-md rounded-[2rem]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black">Rate Your Visit</DialogTitle>
            <DialogDescription>
              Share your experience to help others find the right care.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setReviewOpen(false)} className="rounded-lg">
              Cancel
            </Button>
            <Button
