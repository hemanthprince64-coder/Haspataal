...
export default function PatientDetailsPage() {
    ...
    return (
      <div className="container max-w-5xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-slate-400 font-medium animate-pulse">
          Safely retrieving your clinical profile...
        </p>
      </div>
    );
  }
  ...
  const displayDob = dob
    ? new Date(dob).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
    : '12 Jan 1992';
    ...
    <div className="container max-w-5xl mx-auto px-4 py-8 animate-fade-in space-y-10">
      ...
      <div className="flex items-center justify-between">
        <Button
          asChild
          variant="ghost"
          className="text-slate-500 hover:text-blue-600 gap-2 font-bold px-0"
        >
          <Link href="/profile">
          ...
          <div className="flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              <div className="space-y-1">
                <Badge
                  variant="secondary"
                  className="bg-blue-50 text-blue-700 hover:bg-blue-50 border-0 px-3 py-1 font-bold text-[10px] uppercase tracking-widest"
                >
                  Member since 2023
                </Badge>
                ...
                <p className="text-2xl font-bold text-slate-400">
                  ...
                  <span className="text-blue-600 font-black">
                    HP-{patient?.id ? patient.id.slice(-6).toUpperCase() : '------'}
                  </span>
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {stats.map((stat) => (
          <Card
            key={stat.label}
            className="border-slate-100 shadow-sm rounded-[1.5rem] overflow-hidden group hover:border-blue-200 transition-all"
          >
          ...
          <CardContent className="p-8 pt-6 space-y-6">
            <div className="space-y-0 text-rose-900">
              <DetailItem
                ...
                value={emergencyContactName || 'Guardian'}
              />
              <DetailItem
                icon={<ShieldAlert className="w-4 h-4" />}
                label="Relationship"
                value="Emergency Partner"
              />
              <DetailItem
...
function DetailItem({ icon, label, value }) {
  return (
    <div className="flex flex-col py-5 border-b border-slate-50 last:border-0 gap-1 group">
      ...
      <span className="text-lg font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors">
        {value}
      </span>
    </div>
  );
