...
export default function EditProfile() {
    ...
    return (
      <div className="container max-w-lg mx-auto px-4 py-12 animate-fade-in text-center">
        <Card className="border-emerald-100 shadow-2xl shadow-emerald-500/10 rounded-[2.5rem] bg-white p-10 space-y-6">
          ...
          <Button
            asChild
            className="w-full h-14 rounded-2xl bg-emerald-600 hover:bg-emerald-700 font-bold shadow-lg shadow-emerald-500/20"
          >
            <Link href="/profile">
  ...
  const bloodGroupOptions = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map((v) => ({
    value: v,
    label: v,
  }));
    ...
    <div className="container max-w-2xl mx-auto px-4 py-8 animate-fade-in space-y-8 pb-32">
      <div className="flex items-center justify-between">
        <Button
          asChild
          variant="ghost"
          className="text-slate-500 hover:text-blue-600 font-bold px-0 gap-2"
        >
          <Link href="/profile">
      ...
      <form action={formAction} className="space-y-4">
            ...
            <div className="flex items-center gap-6 pb-2">
              ...
              <div className="flex-1 space-y-2">
                ...
                <Input
                  ...
                  accept="image/*"
                  className="h-10 text-xs p-1.5 cursor-pointer file:font-black file:text-[10px] file:uppercase file:bg-blue-50 file:rounded-lg file:border-0 hover:file:bg-blue-100"
                />
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                defaultValue={p.emergencyContactAltPhone}
              />
            </div>
          </div>
        </Accordion>
        ...
        >
          <div className="space-y-6">
            <FormField
              ...
              defaultValue={p.preferredHospital}
              placeholder="e.g. AIIMS"
            />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField
                label="Care Speciality"
                name="preferredSpeciality"
                defaultValue={p.preferredSpeciality}
                placeholder="e.g. Cardiology"
              />
              <FormField
                label="Regular Consultant"
                name="preferredDoctor"
                defaultValue={p.preferredDoctor}
                placeholder="Dr. Name"
              />
            </div>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-slate-100 z-10 md:static md:bg-transparent md:border-0 md:p-0 pt-6">
          ...
          >
            {isPending ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" /> Digitizing Changes...
              </>
            ) : (
              <>
                <Save className="w-5 h-5" /> Secure & Update Profile
              </>
            )}
          </Button>
        </div>
      ...
      <section className="pt-8 border-t border-slate-100 space-y-6 pb-20">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <p className="text-xs text-slate-400 font-medium">
            Link family members to your master ID for unified care.
          </p>
        </div>
      ...
      {members?.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {members.map((m) => (
            <Card
              key={m.id}
              className="border-slate-100 shadow-sm rounded-2xl overflow-hidden group hover:border-blue-200 transition-all"
            >
              <CardContent className="p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[9px] font-bold uppercase tracking-widest border-slate-100 text-slate-400 px-1.5 py-0 h-4"
                    >
                      {m.relation}
                    </Badge>
                  </div>
                ...
                <form action={deleteFamilyMemberAction}>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-rose-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg shrink-0"
                  >
