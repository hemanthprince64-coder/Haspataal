        ...
        <div className="space-y-8">
            ...
            <Card className="border-none bg-red-600 text-white shadow-2xl shadow-red-500/30 overflow-hidden relative group">
              <CardContent className="p-8 flex flex-col md:flex-row items-center gap-6 relative z-10">
                ...
                <div className="text-center md:text-left">
                  <h3 className="text-3xl font-black tracking-tight mb-2 underline decoration-white/30 underline-offset-8 decoration-4">
                    {t.emergencyTitle}
                  </h3>
                  <p className="text-red-50 text-lg font-bold tracking-tight opacity-90">
                    {t.emergencyDesc}
                  </p>
                </div>
              </CardContent>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <Badge
                className={`${isEmergency ? 'bg-red-500 hover:bg-red-500' : isUrgent ? 'bg-amber-500 hover:bg-amber-500' : 'bg-emerald-500 hover:bg-emerald-500'} text-white font-black text-xs uppercase tracking-widest px-4 py-1.5 rounded-xl border-none shadow-lg shadow-black/5`}
              >
                {isEmergency ? '🚨 ' : isUrgent ? '⚠️ ' : '✅ '} {r.urgency_level}
              </Badge>
              <Badge
                variant="outline"
                className="border-blue-200 bg-blue-50 text-blue-600 font-black text-xs uppercase tracking-widest px-4 py-1.5 rounded-xl"
              >
            ...
            <Card className="border-rose-200 bg-rose-50 shadow-xl shadow-rose-500/5">
              <CardContent className="p-6 flex items-center gap-5">
                ...
                <div className="space-y-1">
                  <h4 className="font-black text-rose-900 uppercase tracking-widest text-xs">
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-lg font-black text-slate-800 tracking-tight flex items-center gap-3">
                  <Calendar className="w-5 h-5 text-blue-600" /> Recommended Specialists in{' '}
                  {formData.city}
                </h3>
              </div>
              {loadingDoctors ? (
                      ...
                      <Card className="rounded-3xl border-slate-100 shadow-xl shadow-slate-200/5 overflow-hidden group-hover:border-blue-300 transition-all group-hover:-translate-y-1">
                        <CardContent className="p-6">
                          <div className="flex justify-between items-start mb-4">
                            <div className="space-y-1">
                              <h4 className="font-black text-lg tracking-tight group-hover:text-blue-600 transition-colors uppercase">
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest mt-1 italic">
            ...
            <Card className="rounded-[2.5rem] bg-indigo-50 border-indigo-100 shadow-xl shadow-indigo-500/5 h-full">
              <CardContent className="p-8 space-y-4">
                ...
                <p className="text-slate-600 font-bold leading-relaxed italic">
                  &quot;{r.ai_reasoning || r.patient_advice}&quot;
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-[2.5rem] bg-white border-slate-200 shadow-md">
            <CardContent className="p-8">
              <div className="flex items-center justify-between mb-8">
                {isEmergency ? (
                  <Button
                    asChild
                    size="lg"
                    className="h-16 px-10 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-black text-lg shadow-xl shadow-red-500/20 active:scale-95 transition-all"
                  >
                    <NextLink href="/emergency">🚑 {t.emergencyTitle}</NextLink>
                  </Button>
                ) : (
                  <Button
                    asChild
                    size="lg"
                    className="h-16 px-10 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
                  >
                    <NextLink
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                ...
                <p className="text-slate-400 text-xs font-bold leading-relaxed uppercase tracking-tight opacity-70 italic">
                  {r.disclaimer}
                </p>
              </div>
            </CardContent>
    ...
    <main className="container max-w-4xl mx-auto px-6 py-8 animate-fade-in text-slate-900 transition-all duration-500">
      ...
      <div className="flex items-center justify-between mb-8">
        <Button
          variant="ghost"
          asChild
          className="text-slate-500 hover:text-blue-600 -ml-4 font-bold"
        >
          <NextLink href="/">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <Badge
              variant="outline"
              className="border-indigo-100 text-indigo-500 font-bold text-[9px] uppercase tracking-widest"
            >
              v2.4 Live
            </Badge>
          </div>
      ...
      <div className="space-y-4 mb-10 px-1">
        ...
        <div className="flex items-center gap-6 justify-center">
          {STEPS(t).map((s, i) => (
            <div
              key={i}
              className={`flex items-center gap-2 transition-all duration-500 ${step === i ? 'opacity-100 scale-100' : 'opacity-30 scale-90'}`}
            >
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs ${step >= i ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'}`}
              >
                {step > i ? '✓' : s.icon}
              </div>
              <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest whitespace-nowrap hidden sm:block">
                {s.label}
              </span>
            </div>
          ))}
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <SelectContent className="rounded-2xl border-slate-100 shadow-xl overflow-hidden">
                      {GENDERS.map((g) => (
                        <SelectItem key={g} value={g} className="font-bold py-4 rounded-xl">
                          {g}
                        </SelectItem>
                      ))}
                    </SelectContent>
            ...
            <CardContent className="p-0 space-y-10">
                ...
                <Textarea
                  placeholder={
                    lang === 'en'
                      ...
                      : 'अपनी स्वास्थ्य समस्याओं के बारे में विस्तार से बताएं...'
                  }
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <SelectContent className="rounded-2xl border-slate-100 shadow-xl overflow-hidden">
                    {DURATIONS.map((d) => (
                      <SelectItem key={d} value={d} className="font-bold py-4 rounded-xl">
                        {d}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <CardContent className="p-0 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${formData.fever === 'yes' ? 'bg-amber-200 text-amber-700' : 'bg-white text-slate-400'}`}
                    >
                  ...
                  <div
                    className={`h-6 w-6 rounded-full border-4 transition-all ${formData.fever === 'yes' ? 'bg-amber-500 border-amber-200 scale-110' : 'bg-white border-slate-200'}`}
                  />
                </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${formData.breathingDifficulty === 'yes' ? 'bg-rose-200 text-rose-700' : 'bg-white text-slate-400'}`}
                    >
                  ...
                  <div
                    className={`h-6 w-6 rounded-full border-4 transition-all ${formData.breathingDifficulty === 'yes' ? 'bg-rose-500 border-rose-200 scale-110' : 'bg-white border-slate-200'}`}
                  />
                </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${formData.seizure === 'yes' ? 'bg-rose-200 text-rose-700' : 'bg-white text-slate-400'}`}
                    >
                  ...
                  <div
                    className={`h-6 w-6 rounded-full border-4 transition-all ${formData.seizure === 'yes' ? 'bg-rose-500 border-rose-200 scale-110' : 'bg-white border-slate-200'}`}
                  />
                </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${formData.consciousnessNormal === 'no' ? 'bg-rose-200 text-rose-700' : 'bg-white text-slate-400'}`}
                    >
                      <ShieldAlert className="w-6 h-6" />
                    </div>
                    <span className="font-black text-slate-700 tracking-tight">
                      {t.consciousnessLabel}
                    </span>
                  </div>
                  <div
                    className={`h-6 w-6 rounded-full border-4 transition-all ${formData.consciousnessNormal === 'no' ? 'bg-rose-500 border-rose-200 scale-110' : 'bg-white border-slate-200'}`}
                  />
                </div>
              </div>
