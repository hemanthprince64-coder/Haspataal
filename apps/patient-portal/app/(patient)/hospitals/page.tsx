      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium tracking-tight">
            Verified healthcare centers in <span className="text-blue-600 font-black">{city}</span>
          </p>
        </div>
      ...
      {hospitalsWithData.length === 0 ? (
        <div className="py-32 text-center bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-200/60 animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-slate-500 text-lg font-medium max-w-sm mx-auto leading-relaxed">
            We are currently expanding our network in{' '}
            <span className="font-black text-slate-700">{city}</span>. Please check again soon.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {hospitalsWithData.map((hospital) => (
            <Card
              key={hospital.id}
              className="group rounded-2xl border-slate-200/60 shadow-lg shadow-slate-200/5 bg-white overflow-hidden hover:border-blue-300 hover:shadow-xl transition-all duration-500 relative"
            >
              ...
              <CardContent className="p-6">
                <Link href={`/hospitals/${hospital.id}`} className="block mb-3">
                  <h3 className="font-black text-xl tracking-tight leading-tight group-hover:text-blue-600 transition-colors uppercase line-clamp-1">
                    {hospital.name}
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    asChild
                    variant="outline"
                    className="h-12 rounded-xl border-slate-200 font-bold text-xs hover:bg-slate-50 transition-all"
                  >
                    <Link href={`/hospitals/${hospital.id}`}>Details</Link>
                  </Button>
                  <Button
                    asChild
                    className="h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-500/10 transition-all active:scale-95"
                  >
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight text-slate-900">
                Cant find a hospital?
              </h3>
              <p className="text-slate-500 font-medium max-w-sm">
                Contact our 24/7 assistance hub for personal coordination.
              </p>
            </div>
          </div>
          <Button
            variant="outline"
            size="lg"
            className="h-14 px-8 rounded-xl border-slate-200 font-black text-sm uppercase tracking-widest hover:bg-white transition-all"
          >
