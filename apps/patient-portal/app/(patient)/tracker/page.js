  ...
  return (
    <main className="container max-w-4xl mx-auto px-6 py-10 animate-fade-in text-slate-900">
      <Button
        asChild
        variant="ghost"
        className="mb-8 text-slate-500 hover:text-pink-600 -ml-4 font-bold"
      >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Badge
              variant="secondary"
              className="text-pink-700 bg-pink-100/50 hover:bg-pink-100 border-pink-200 px-3 py-1 font-bold text-[10px] uppercase tracking-[0.2em] backdrop-blur-sm"
            >
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Dedicated tracking for every phase of your journey.
          </p>
        </div>
        <Button
        ...
        >
          {showForm ? (
            ...
            <>
              <Pencil className="w-6 h-6 mr-3" /> Edit Profile
            </>
          )}
        </Button>
      </div>
        ...
        <CardContent className="w-full bg-white/95 backdrop-blur-sm rounded-[2.35rem] p-8 md:p-12 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
          ...
          <div className="flex-1 text-center md:text-left">
            <Badge className="bg-emerald-500 hover:bg-emerald-500 text-white font-black text-[9px] uppercase tracking-widest px-3 py-1.5 rounded-lg border-none mb-4 shadow-lg">
              {pregnancyProfile?.gestationalAge
            ...
            <p className="text-slate-500 text-lg font-medium tracking-tight mb-0">
              {pregnancyProfile?.edd
                ? `Expected Delivery: ${new Date(pregnancyProfile.edd).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}`
                : 'Keep your clinical profile updated for personalized care nudges.'}
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="mb-12 rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            ...
            <CardDescription className="text-slate-500 font-medium tracking-tight">
              Updating this syncs with your doctor&apos;s recovery dashboard.
            </CardDescription>
          </CardHeader>
            ...
            <form action={formAction} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    type="date"
                    defaultValue={
                      pregnancyProfile.lmp
                        ? new Date(pregnancyProfile.lmp).toISOString().split('T')[0]
                        : ''
                    }
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    defaultValue={
                      pregnancyProfile.edd
                        ? new Date(pregnancyProfile.edd).toISOString().split('T')[0]
                        : ''
                    }
                    className="h-14 rounded-2xl border-slate-200 bg-slate-50/50 focus-visible:ring-pink-500/20 focus-visible:ring-4 font-bold transition-all"
                  />
              ...
              <div className="flex items-center space-x-3 bg-red-50 p-6 rounded-2xl border border-red-100 group">
                <input
                  ...
                  name="highRisk"
                  value="true"
                  defaultChecked={pregnancyProfile.highRisk}
                  id="highRisk"
                  className="h-6 w-6 rounded-lg accent-rose-600"
                />
                <Label
                  htmlFor="highRisk"
                  className="font-black text-rose-700 cursor-pointer text-base"
                >
                  Mark as High-Risk Pregnancy <ShieldAlert className="inline w-5 h-5 ml-1" />
                </Label>
              </div>
              ...
              >
                {isPending ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Virtualizing Profile...
                  </>
                ) : (
