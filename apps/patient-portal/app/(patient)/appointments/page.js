  ...
  return (
    <div className="container max-w-5xl mx-auto px-4 py-8 animate-fade-in">
      <Button
        asChild
        variant="ghost"
        className="mb-6 text-slate-500 hover:text-blue-600 -ml-4 font-bold"
      >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Manage your consultations and check health history.
          </p>
        </div>
        <Button
          asChild
      ...
      <div className="bg-slate-100/50 p-1.5 rounded-[1.75rem] flex gap-2 mb-10 max-w-md border border-slate-200/50 backdrop-blur-sm">
        ...
        >
          <Clock className={`w-4 h-4 ${tab === 'upcoming' ? 'animate-pulse' : ''}`} /> Upcoming (
          {upcoming.length})
        </button>
        <button
          onClick={() => setTab('past')}
