  ...
  return (
    <main className="container max-w-4xl mx-auto px-6 py-10 animate-fade-in text-slate-900">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Manage your addresses for home visits and medicine deliveries.
          </p>
        </div>
        <Button
        ...
        >
          {showAddForm ? (
            ...
            <>
              <Plus className="w-6 h-6 mr-3" /> Add Location
            </>
          )}
        </Button>
      </div>
        ...
        <Card className="mb-12 rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            ...
            <CardDescription className="text-slate-500 font-medium tracking-tight">
              Used for sample collections and pharmacy logistics.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <form onSubmit={handleFormSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Select name="type" defaultValue="Home">
                    ...
                    <SelectContent className="rounded-2xl border-slate-200 shadow-xl">
                      <SelectItem value="Home" className="font-bold py-3 rounded-xl">
                        🏠 Home
                      </SelectItem>
                      <SelectItem value="Work" className="font-bold py-3 rounded-xl">
                        🏢 Work
                      </SelectItem>
                      <SelectItem value="Other" className="font-bold py-3 rounded-xl">
                        📍 Other
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    className="h-14 rounded-2xl border-slate-200 bg-slate-50/50 focus-visible:ring-blue-500/20 focus-visible:ring-4 font-bold transition-all"
                  />
                </div>
                <div className="md:col-span-2 space-y-3">
                  ...
                  <Textarea
                    ...
                    className="min-h-[100px] rounded-2xl border-slate-200 bg-slate-50/50 focus-visible:ring-blue-500/20 focus-visible:ring-4 font-bold transition-all"
                  />
                </div>
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
              ...
              <div className="flex items-center space-x-3 bg-slate-50 p-6 rounded-2xl border border-slate-100 group cursor-pointer">
                ...
                <Label
                  htmlFor="isDefault"
                  className="font-black text-slate-700 cursor-pointer text-base"
                >
                  Set as Primary Recovery Address
                </Label>
              </div>
              ...
              >
                {isSaving ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Saving Location...
                  </>
                ) : (
                  'Register Address'
                )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {loading ? (
          [1, 2, 3, 4].map((i) => (
            <Skeleton key={i} className="h-64 rounded-[2.5rem] bg-slate-100" />
          ))
        ) : (
          <>
            ...
            {addresses.map((addr) => (
              <Card
                key={addr.id}
                className="group rounded-[2.5rem] border-slate-200/60 shadow-xl shadow-slate-200/5 bg-white overflow-hidden hover:border-blue-300 hover:shadow-2xl hover:shadow-blue-500/5 transition-all duration-500 relative"
              >
                {addr.isDefault && (
                ...
                <CardContent className="p-8">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h3 className="font-black text-2xl tracking-tight group-hover:text-blue-600 transition-colors">
                        {addr.type}
                      </h3>
                      <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-0.5">
                        Verified Location
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-8 ml-2">
                    <p className="text-slate-600 font-bold leading-relaxed line-clamp-2">
                      {addr.address}
                    </p>
                    {addr.landmark && (
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                    {!addr.isDefault ? (
                      <Button
                        ...
                        variant="ghost"
                        className="text-xs font-black uppercase tracking-widest text-blue-600 hover:text-blue-700 hover:bg-blue-50 px-4 rounded-xl"
                      >
                        Set as Default
                      </Button>
                    ) : (
