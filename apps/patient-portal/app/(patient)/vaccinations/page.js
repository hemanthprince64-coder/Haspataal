      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Badge
              variant="secondary"
              className="text-teal-700 bg-teal-100/50 hover:bg-teal-100 border-teal-200 px-3 py-1 font-bold text-[10px] uppercase tracking-[0.2em] backdrop-blur-sm"
            >
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Digital immunization records for you and your family.
          </p>
        </div>
        <Button
        ...
        >
          {showForm ? (
            ...
            <>
              <Plus className="w-6 h-6 mr-3" /> Add Certificate
            </>
          )}
        </Button>
      </div>
        ...
        <CardContent className="w-full bg-white/95 backdrop-blur-sm rounded-[1.85rem] p-6 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="flex-1">
            <p className="text-slate-600 font-bold leading-relaxed">
              <span className="text-teal-600 font-black uppercase tracking-widest text-[10px] block mb-1">
                Immunization Shield
              </span>
              Maintaining accurate vaccination records is critical for school, travel, and long-term
              immunity tracking.
            </p>
          </div>
        </CardContent>
        ...
        <Card className="mb-12 rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            <CardTitle className="text-2xl font-black text-slate-900 tracking-tight">
              New Immunization Entry
            </CardTitle>
            <CardDescription className="text-slate-500 font-medium tracking-tight">
              Record details from your vaccination certificate.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <form action={formAction} className="space-y-8">
              ...
              >
                {isPending ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Digitizing Certificate...
                  </>
                ) : (
                  'Save Immunization'
                )}
