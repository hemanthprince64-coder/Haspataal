      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Badge
              variant="secondary"
              className="text-rose-700 bg-rose-100/50 hover:bg-rose-100 border-rose-200 px-3 py-1 font-bold text-[10px] uppercase tracking-[0.2em] backdrop-blur-sm"
            >
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Track your core metrics and monitor recovery trends.
          </p>
        </div>
        <Button
        ...
        >
          {showForm ? (
            ...
            <>
              <Plus className="w-6 h-6 mr-3" /> Record Vitals
            </>
          )}
        </Button>
      </div>
      ...
      <div className="grid gap-8">
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <Skeleton key={i} className="h-32 rounded-3xl bg-slate-100" />
              ))}
            </div>
          ) : (
          ...
          <Card className="rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
            <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
              ...
              <CardDescription className="text-slate-500 font-medium tracking-tight">
                Capture your current vitals for Clinical history.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-8">
              <form action={formAction} className="space-y-10">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="relative">
                        <Input
                          ...
                          className="h-14 rounded-2xl border-slate-200 bg-slate-50/30 focus-visible:ring-rose-500/20 focus-visible:ring-4 font-bold"
                        />
                        <span className="absolute right-4 top-4 text-[10px] font-black text-slate-300">
                          KG
                        </span>
                      </div>
                      <div className="relative">
                ...
                >
                  {isPending ? (
                    <>
                      <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Syncing Metrics...
                    </>
                  ) : (
                    'Save Health Data'
                  )}
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <Skeleton key={i} className="h-40 rounded-[2rem] bg-slate-100" />
              ))}
            </div>
          ) : (
