      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Active list of your prescribed and over-the-counter drugs.
          </p>
        </div>
        <Button
        ...
        >
          {showForm ? (
            ...
            <>
              <Plus className="w-6 h-6 mr-3" /> Add Medication
            </>
          )}
        </Button>
      </div>
        ...
        <CardContent className="w-full bg-white/95 backdrop-blur-sm rounded-[1.85rem] p-6 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="flex-1">
            <p className="text-slate-600 font-bold leading-relaxed">
              ...
              Comprehensive tracking prevents harmful drug interactions. Always keep this list
              updated.
            </p>
          </div>
        </CardContent>
        ...
        <Card className="mb-12 rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            ...
            <CardDescription className="text-slate-500 font-medium tracking-tight">
              Enter exact details as prescribed by your doctor.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <form action={addAction} className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <Select name="frequency">
                    ...
                    <SelectContent className="rounded-2xl border-slate-200 shadow-xl">
                      <SelectItem value="OD" className="font-bold py-3 rounded-xl">
                        Once Daily (OD)
                      </SelectItem>
                      <SelectItem value="BD" className="font-bold py-3 rounded-xl">
                        Twice Daily (BD)
                      </SelectItem>
                      <SelectItem value="TDS" className="font-bold py-3 rounded-xl">
                        Thrice Daily (TDS)
                      </SelectItem>
                      <SelectItem value="QID" className="font-bold py-3 rounded-xl">
                        Four Times (QID)
                      </SelectItem>
                      <SelectItem value="SOS" className="font-bold py-3 rounded-xl">
                        As Needed (SOS)
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              >
                {isAdding ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Loggin Medication...
                  </>
                ) : (
                  'Save to My MedList'
                )}
