...
const pricingSchema = z
  .object({
    ...
    isInstrumentBased: z.boolean().default(false),
  })
...
function formatPricing(row: any) {
  ...
  return {
    ...
    sampleType: row.test?.sampleType,
    fastingRequired: row.test?.fastingRequired,
    preparationInstructions: row.test?.preparationInstructions,
    referenceRanges: row.test?.referenceRanges,
    equipmentCode: row.test?.equipmentCode,
    isInstrumentBased: row.test?.isInstrumentBased,
  };
}
...
export async function POST(req: NextRequest) {
  ...
  const price = data.patientMrp ?? data.hospitalPrice;
  if (!price || price <= 0) {
    ...
    (await prisma.$transaction(async (tx) => {
      ...
      const test = await tx.diagnosticMasterTest.create({
        data: {
          ...
          turnaroundTimeHours: data.turnaroundHours ?? 24,
          ...
          isInstrumentBased: data.isInstrumentBased,
        },
      });
  ...
  const test = await prisma.hospitalDiagnosticPricing.upsert({
    ...
    update: {
      price,
      tatOverrideHours: data.turnaroundHours,
      isAvailable: true,
    },
    create: {
