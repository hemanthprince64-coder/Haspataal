      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-slate-900 to-indigo-950 p-8 text-white shadow-2xl">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-3">
            <span
              className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                journey.seriousness === 'Urgent'
                  ? 'bg-red-500 text-white'
                  : journey.seriousness === 'Monitor'
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight leading-tight max-w-2xl">
            Your recovery plan for{' '}
            <span className="text-indigo-300">{journey.conditionSimple}</span>
          </h1>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-white/10">
            <div className="flex -space-x-2">
              <div className="w-8 h-8 rounded-full bg-indigo-500 border-2 border-slate-900 flex items-center justify-center text-[10px]">
                🩺
              </div>
              <div className="w-8 h-8 rounded-full bg-emerald-500 border-2 border-slate-900 flex items-center justify-center text-[10px]">
                🤖
              </div>
            </div>
            <p className="text-xs text-slate-400">
              ...
              <span className="text-white font-bold">
                {journey.visit.appointment?.doctor.fullName || 'Consultant'}
              </span>{' '}
              & Haspataal AI
            </p>
          </div>
          ...
          <div className="rounded-[2.5rem] bg-white border border-slate-100 shadow-soft p-8">
            ...
            <div className="space-y-4">
              {journey.medications.map((med) => (
                <div
                  key={med.id}
                  className="group relative overflow-hidden rounded-3xl bg-slate-50 border border-slate-100 p-5 hover:bg-white hover:border-indigo-100 hover:shadow-lg transition-all duration-300"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div
                        ...
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl cursor-pointer transition-all duration-300 ${
                          medsTaken[med.id]
                            ...
                            : 'bg-white border border-slate-200 text-slate-300 hover:border-indigo-300'
                        }`}
                      >
                        {medsTaken[med.id] ? '✓' : '💊'}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={`px-4 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 ${med.morning ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' : 'bg-slate-100 text-slate-400 opacity-40'}`}
                      >
                        <span>☀️ Morning</span>
                      </div>
                      <div
                        className={`px-4 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 ${med.afternoon ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-slate-100 text-slate-400 opacity-40'}`}
                      >
                        <span>🌤️ P.M.</span>
                      </div>
                      <div
                        className={`px-4 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 ${med.night ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400 opacity-40'}`}
                      >
                        <span>🌙 Night</span>
                      </div>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="rounded-[2.5rem] bg-rose-50 border border-rose-100 p-8 space-y-6">
            ...
            <div className="space-y-3">
              {journey.redFlags.map((flag, idx) => (
                <div
                  key={idx}
                  className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 border border-rose-200"
                >
          ...
          {journey.followUp && (
            <div className="rounded-[2.5rem] bg-indigo-600 p-8 text-white shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              ...
              <p className="text-sm text-indigo-100 mb-6 opacity-90">{journey.followUp.reason}</p>
