...
const ContinuousCareHub: React.FC<ContinuousCareHubProps> = ({ data }) => {
  ...
  return (
    <div className="w-full space-y-8 animate-fade-in pb-20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="max-w-xl">
            ...
            <h1 className="text-4xl sm:text-5xl font-black tracking-tight leading-tight mb-4">
              Day <span className="text-medical-400">{data.currentDay}</span>{' '}
              <span className="text-slate-500 text-3xl font-medium">of 14</span>
            </h1>
            <p className="text-lg text-slate-400 font-medium leading-relaxed">
              Treating <span className="text-white">{data.condition}</span>. You are halfway to full
              recovery.
            </p>
          </div>
            ...
            <div className="relative w-32 h-32 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-black">
                  {Math.round((data.currentDay / 14) * 100)}%
                </span>
      ...
      {data.drift?.drifted && (
        <div
          className={`p-6 rounded-[2.5rem] border-2 animate-pulse-subtle shadow-lg ${
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center sm:text-left space-y-1">
              <h3 className="text-xl font-black tracking-tight leading-tight">
                AI Health Alert:{' '}
                {data.drift.reason === 'NEGATIVE_TREND'
                  ? 'Negative Trend Detected'
                  : 'Recovery Stagnation'}
              </h3>
              <p className="text-sm font-medium opacity-80 leading-relaxed uppercase tracking-widest text-[10px]">
                Our Care Engine has detected that your symptoms are not improving as expected for
                Day {data.currentDay}.
                <strong className="ml-1"> {data.drift.action?.replace(/_/g, ' ')}</strong> is highly
                advised to ensure safe recovery.
              </p>
            </div>
            <button
              className={`px-8 py-4 rounded-2xl font-black text-sm shadow-xl transition-all hover:scale-105 active:scale-95 whitespace-nowrap ${
          ...
          {data.fullHistory.map((step) => (
            ...
            >
              <span className="text-[10px] font-black uppercase">
                {step.dayNumber === data.currentDay ? 'Today' : `D-${step.dayNumber}`}
              </span>
                ...
                <div className="w-full sm:w-80 space-y-4">
                  ...
                  <div className="space-y-3">
                    {data.medications.map((med) => (
                      <div
                        key={med.id}
                        className="p-4 rounded-2xl bg-slate-900 text-white shadow-lg space-y-3"
                      >
                        ...
                        <div className="flex gap-2">
                          {med.morning && (
                            <button
                              onClick={() => handleLogMed(med.medName, 'Morning')}
                              className="flex-1 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-[9px] font-bold transition-colors"
                            >
                              ☀️ AM
                            </button>
                          )}
                          {med.afternoon && (
                            <button
                              onClick={() => handleLogMed(med.medName, 'Afternoon')}
                              className="flex-1 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-[9px] font-bold transition-colors"
                            >
                              🌦️ PM
                            </button>
                          ...
                          {med.night && (
                            <button
                              onClick={() => handleLogMed(med.medName, 'Night')}
                              className="flex-1 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-[9px] font-bold transition-colors"
                            >
                              🌙 NT
                            </button>
                  ...
                  <div className="p-6 rounded-[2rem] bg-indigo-50 border border-indigo-100 flex flex-col items-center text-center">
                    ...
                    <div className="flex gap-3 justify-center w-full">
                      <button
                        ...
                        className="w-12 h-12 rounded-2xl bg-white shadow-soft text-xl hover:scale-110 active:scale-95 transition-all"
                      >
                        🚀
                      </button>
                      <button
                      ...
                      <button
                        ...
                        className="w-12 h-12 rounded-2xl bg-rose-500 text-white shadow-lg text-xl hover:scale-110 active:scale-95 transition-all"
                      >
                        🤒
                      </button>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-3 font-medium">
                      Response updates your urgency score
                    </p>
                  </div>
                </div>
              )}
