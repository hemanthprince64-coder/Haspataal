...
import { loginDoctor, loginDoctorWithOtp, requestDoctorOtp } from '@/app/actions';
...
export default function DoctorLogin() {
  ...
  const handlePasswordSubmit = (e) => {
    ...
    startPasswordTransition(async () => {
      try {
        const res = await loginDoctor(null, formData);
        if (res && res.message) {
  ...
  const handleSendOtp = () => {
    ...
    startOtpTransition(async () => {
      ...
      const res = await requestDoctorOtp(null, data);
      if (res.success) {
        setOtpSent(true);
        setOtpError('');
  ...
  const handleOtpSubmit = (e) => {
    ...
    startOtpLoginTransition(async () => {
      try {
        const res = await loginDoctorWithOtp(null, formData);
        if (res.success) {
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="bg-teal-600 hover:bg-teal-700 text-white font-bold h-12 px-4 rounded-xl shrink-0 disabled:opacity-50 disabled:pointer-events-none"
                    >
                      {isOtpPending ? (
                ...
                {otpSent && (
                  <div className="space-y-1.5 animate-in slide-in-from-top-3 fade-in duration-300">
                    ...
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                        <Key className="h-4 w-4 text-slate-400 group-focus-within:text-teal-500 transition-colors" />
                      </div>
                      <Input
                        id="otp"
                        name="otp"
                        ...
                        maxLength={6}
                        required
                        className="pl-11 h-12 bg-slate-50/50 border-slate-200 focus-visible:ring-teal-500/20 focus-visible:border-teal-500/50 text-[15px] transition-all tracking-[0.25em] font-mono font-bold"
                        placeholder="••••••"
                      />
                    </div>
                  </div>
