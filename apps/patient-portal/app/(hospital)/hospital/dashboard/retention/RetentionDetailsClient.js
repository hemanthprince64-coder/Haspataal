...
export default function RetentionDetailsClient({ hospitalId }) {
  ...
  return (
    <div style={{ display: 'grid', gap: '1.5rem' }}>
      ...
      >
        <h3
          style={{ fontSize: '1rem', fontWeight: '700', color: '#92400e', marginBottom: '0.5rem' }}
        >
          Retention Insights
        </h3>
        <p style={{ fontSize: '0.875rem', color: '#b45309' }}>
          Your overall retention rate of <strong>{kpi?.retentionRate}%</strong> is{' '}
          {kpi?.deltaVsLastMonth > 0 ? 'up by ' + kpi.deltaVsLastMonth + '%' : 'down'} compared to
          last month. The network average in Patna is 63%.
        </p>
      </div>
    </div>
  );
