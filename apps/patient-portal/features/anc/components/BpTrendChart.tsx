      ...
      <Card className="rounded-[2rem] border-slate-200/60 shadow-xl overflow-hidden">
        <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6">
          ...
          <CardDescription className="text-slate-500 font-medium">
            Blood pressure monitoring for PIH detection
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="rounded-[2rem] border-slate-200/60 shadow-xl overflow-hidden">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        {pihResult.isPih && (
          <div
            className={`p-4 rounded-xl border ${pihResult.isSevere ? 'bg-rose-50 border-rose-200' : 'bg-amber-50 border-amber-200'}`}
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="font-black text-slate-800">
                {pihResult.isSevere
                  ...
                  : 'Moderate Hypertension Detected'}
              </p>
            </div>
        ...
        <div className="space-y-3">
          {bpReadings.map((reading, idx) => {
            ...
            const isHigh = (reading.bpSystolic ?? 0) >= 140 || (reading.bpDiastolic ?? 0) >= 90;
            return (
              <div
                key={idx}
                className={`flex items-center gap-4 p-3 rounded-xl border ${isLatest ? 'bg-pink-50 border-pink-200' : 'bg-slate-50 border-slate-100'}`}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${isHigh ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}
                >
                  {isHigh ? (
                ...
                <div className="flex-1">
                  <p className="font-bold text-sm text-slate-800">
                    {new Date(reading.visitDate).toLocaleDateString('en-IN', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    })}{' '}
                    — Week {reading.gestationalAge || '?'}
                  </p>
                  <p
                    className={`text-xs font-bold ${isHigh ? 'text-rose-600' : 'text-emerald-600'}`}
                  >
                ...
                {isLatest && pihResult.isPih && (
                  <Badge
                    className={`${pihResult.isSevere ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}
                  >
                    {pihResult.severity}
                  </Badge>
                )}
        ...
        <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
            Threshold Reference (Bihar NHM Protocol)
          </p>
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-slate-600">
                Moderate: ≥140/90 (twice, 4h apart)
              </span>
            </div>
            <div className="flex items-center gap-2">
