...
const DISCLAIMER =
  'MedChat AI provides triage guidance for informational purposes only. It does not provide medical diagnosis or treatment. Please consult a qualified doctor for proper evaluation. If symptoms worsen, seek immediate emergency care.';
const AI_SERVICE_URL = process.env.MEDCHAT_AI_SERVICE_URL || 'http://localhost:8000/triage';

/**
 * Call External Python Triage Service (Gemini 2.5)
 */
async function callExternalAiService(patientData) {
  try {
    ...
    if (response.ok) {
      ...
      return { ...result, is_ai_powered: true };
    }
  } catch (error) {
    console.warn(
      'External AI Service unreachable, falling back to local Gemini 2.0:',
      error.message,
    );
  }
  return null;
...
export async function triagePatient(input) {
  ...
  // 5. Hybrid Logic: Call External Python Service (Gemini 2.5) first, then local Gemini 2.0
  let aiResult = await callExternalAiService(input);
  if (!aiResult) {
    aiResult = await triageWithAI(input);
  }
  ...
  // ── BUILD CATEGORIES ────────────────────────────────────
  const categories = matchedCategories.size > 0 ? [...matchedCategories] : ['General symptoms'];
  ...
  const clinicalSummary =
    aiResult?.clinical_summary_for_doctor ||
  ...
  // ── PATIENT ADVICE ──────────────────────────────────────
  const patientAdvice =
    aiResult?.patient_advice || buildPatientAdvice(urgencyLevel, redFlagDetected, isPediatric);
  ...
  const toonString = Buffer.from(JSON.stringify(ehrTransmissionData)).toString('base64');
...
function buildClinicalSummary(ctx) {
  const lines = [];
  const ageLabel = ctx.isInfant
    ? `${ctx.age}-year-old infant`
    : ctx.isPediatric
      ? `${ctx.age}-year-old pediatric patient`
  ...
  if (ctx.redFlagDetected) {
    lines.push(
      `RED FLAGS IDENTIFIED: ${ctx.detectedRedFlags.join('; ')}. Immediate evaluation warranted.`,
    );
  }
...
function buildPatientAdvice(urgencyLevel, redFlagDetected, isPediatric) {
  ...
  if (urgencyLevel === 'URGENT') {
    const base =
      'Your symptoms suggest that evaluation by a doctor is recommended within the next few hours. Please schedule a consultation at the earliest convenience through Haspataal or visit a nearby hospital.';
    if (isPediatric) {
      return (
        base +
        ' Since the patient is a child, prompt medical evaluation is especially important. Monitor closely for any worsening signs.'
      );
    }
    return (
      base +
      ' In the meantime, rest adequately and stay hydrated. If symptoms worsen, seek immediate emergency care.'
    );
  }

  // ROUTINE
  const base =
    'These symptoms can often be managed with a scheduled consultation. We recommend booking an appointment with the suggested speciality through Haspataal for proper evaluation.';
  if (isPediatric) {
    return (
      base +
      ' For children, monitoring symptom progression is important. If any new or worsening symptoms develop, please seek medical attention promptly.'
    );
  }
  return (
    base +
    ' In the meantime, maintain adequate hydration and rest. If your symptoms worsen or new symptoms develop, seek medical attention promptly.'
  );
}
