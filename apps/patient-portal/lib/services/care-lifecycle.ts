...
export class CareLifecycleService {
  ...
  static async logMedicationForPatient(
    patientId: string,
  ...
  static async submitCheckIn(
    ...
    logger.info(
      { action: 'submit_checkin', careJourneyId, dayNumber, status },
      'Patient submitted recovery status',
    );
    ...
    const checkIn = await prisma.careCheckIn.create({
      data: {
        ...
        analysis:
          status === 'WORSE'
            ? 'AI Escalation Required: Patient reported worsening symptoms.'
            : 'Recovery tracking in progress.',
      },
    });
  ...
  static async submitCheckInForPatient(
    patientId: string,
    careJourneyId: string,
    dayNumber: number,
    status: 'BETTER' | 'SAME' | 'WORSE',
  ...
  static async analyzeRecoveryDrift(careJourneyId: string) {
    ...
    const isPersistentStagnation =
      lastTwo.length >= 2 && lastTwo.every((c) => c.status !== 'BETTER');
