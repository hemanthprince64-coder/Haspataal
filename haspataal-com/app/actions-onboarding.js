  ...
  try {
    ...
    const [existingHospital, existingAdminMobile, existingAdminEmail] = await Promise.all([
      prisma.hospitalsMaster.findUnique({
        where: { registrationNumber: rawData.registrationNumber },
      }),
    ...
    if (existingAdminMobile)
      return { success: false, message: 'Admin mobile number is already registered.' };
    if (existingAdminEmail)
      return { success: false, message: 'Admin email is already registered.' };
    ...
    if (e.code === 'P2002') {
      return {
        success: false,
        message: 'Registration number or Admin mobile/email already exists.',
      };
    }
