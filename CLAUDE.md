...
- **Prisma CLI Wrapper Must Use Explicit Function Reference:** `scripts/prisma-cmd.js` originally used `require('./prepare-schema')` and then called `prepareSchema()` via side-effect hoisting, which is fragile under module bundlers and Node ESM/CJS interop. Fixed by importing the module as `const prepareSchema = require('./prepare-schema');` and calling `prepareSchema.prepareSchema();` explicitly. _(Fixed 2026-06-14)_
