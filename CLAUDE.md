...
- **UI/UX Audit — Patient Portal:** Comprehensive audit fixing: (1) Login page error states with red borders and Lucide icons, (2) Search page error boundary with retry, (3) Booking flow slot validation and wallet payment error UI, (4) Profile logout button overlap with BottomNav, (5) Emergency SOS mock API dispatch, (6) MedChat tooltip on AI speciality badges. _(Implemented 2026-05-17)_
- **Accessibility — BottomNav & Sidebar:** Added `aria-label` to all BottomNav links. Replaced custom `focus-ring` class with standard `focus-visible:ring-2` in Sidebar links and buttons. Removed `suppressHydrationWarning` from interactive elements. Fixed Sidebar active-link layout shift by using persistent `border-l-[3px] border-transparent` on inactive items. _(Fixed 2026-05-17)_
- **Design System — Typography Standardization:** Migrated DoctorCard from arbitrary pixel values (`text-[9px]`, `text-[13px]`, `text-[7px]`) to standard Tailwind tokens (`text-xs`, `text-sm`). This ensures consistent rendering across devices and eliminates sub-pixel rounding issues on mobile. _(Standardized 2026-05-17)_
- **Doctor Profile Loading/NotFound States:** The `doctor/[id]/page.js` now has proper loading spinner and "Doctor Not Found" error states instead of hardcoded static data. Uses `useState`/`useEffect` to simulate async fetch with graceful fallback. _(Implemented 2026-05-17)_
- **Lucide-react Brand Icons Removed:** `Facebook`, `Twitter`, `Instagram`, and `Linkedin` icons no longer exist in modern `lucide-react`. Use generic alternatives (`Users`, `MessageCircle`, `Share2`) or SVG brand icons from a separate package. _(Fixed 2026-05-17)_
