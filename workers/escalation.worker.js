...
const notification_curfew_1 = require("../lib/notification-curfew");
const redis_1 = __importDefault(require("../lib/redis"));
const event_service_1 = require("../services/event.service");
// ---- Prometheus counter (best-effort; skip if prom-client not present) --------
let escalationCounter = null;
try {
...
function recordMetric(hospitalId, chronicTag, acknowledged) {
...
const _pgPool = new pg_1.Pool({ connectionString: process.env.DATABASE_URL });
class EscalationWorker {
    ...
    static async processQueue() {
        ...
        try {
         ...
         LIMIT $1
         FOR UPDATE SKIP LOCKED`, [BATCH_LIMIT]);
            if (res.rowCount === 0) {
                console.log('[EscalationWorker] No unprocessed escalation alerts.');
            ...
            for (const alert of res.rows) {
                ...
                if (redis_1.default) {
                    const acquired = await redis_1.default.eval(LUA_ACQUIRE, 1, lockKey, String(LOCK_TTL_SECONDS));
                    if (!acquired) {
                        console.warn(`[EscalationWorker] Skipping alert ${alert.id.slice(0, 8)} — ` +
                ...
                try {
                    ...
                    if (redis_1.default)
                        await redis_1.default.eval(LUA_RELEASE, 1, lockKey);
                }
            }
            await client.query('COMMIT');
        }
    ...
    static async attemptEscalatePrisma(alert) {
        ...
        const curfewDecision = (0, notification_curfew_1.evaluateCurfew)(new Date());
        if (curfewDecision === 'DEFER') {
            console.warn(`[EscalationWorker] Alert ${id.slice(0, 8)} deferred due to notification curfew.`);
            return;
        }
        let sent = false;
        let channel = 'pending';
        try {
            ...
            if (sent) {
                ...
                await event_service_1.EventService.publish('escalation_notification_sent', { alertId: id, channel, missedCount, chronicTag }, hospitalId, patientId);
            ...
            if (attempts >= 3) {
                ...
                console.error(`[EscalationWorker] Alert ${id.slice(0, 8)} dead-lettered after ${attempts} attempts:`, err.message);
                await event_service_1.EventService.publish('escalation_dlq', { alertId: id, reason: err.message }, hospitalId, patientId);
            }
            else {
                await db_1.prisma.escalationAlert.update({
                    where: { id },
                ...
                console.warn(`[EscalationWorker] Alert ${id.slice(0, 8)} attempt ${attempts} failed; retrying next cycle.`);
            }
        }
    }
    static async attemptEscalate(client, alert) {
        const { id, hospital_id: hospitalId, patient_id: patientId, missed_count: missedCount, chronic_tag: chronicTag, patient_name: patientName, doctor_name: doctorName, doctor_phone: doctorPhone, hospital_name: hospitalName, } = alert;
        const message = buildEscalationMessage(patientName, doctorName, hospitalName, missedCount, chronicTag);
        // ── Curfew check ─────────────────────────────────────────────────
        const curfewDecision = (0, notification_curfew_1.evaluateCurfew)(new Date());
        if (curfewDecision === 'DEFER') {
        ...
        try {
            ...
            if (sent) {
                await client.query(`UPDATE "escalation_alerts"
             SET "notification_sent" = true,
                 "sent_via"          = $1,
                 "sent_at"           = now(),
                 "updated_at"        = now()
           WHERE id = $2`, [channel, id]);
                recordMetric(hospitalId, chronicTag || 'UNKNOWN', true);
                await event_service_1.EventService.publish('escalation_notification_sent', { alertId: id, channel, missedCount, chronicTag }, hospitalId, patientId);
                console.log(`[EscalationWorker] ✓ Alert ${id.slice(0, 8)} sent via ${channel}`);
            }
     ...
     */
    static async mockProviderSend(channel, phone, message) {
        const ok = Math.random() > 0.2;
        console.log(`[EscalationWorker/MockProvider] ${channel.toUpperCase()} → ${phone}: "${message.slice(0, 50)}…" → ${ok ? 'SUCCESS' : 'FAIL'}`);
        return ok;
    }
}
exports.EscalationWorker = EscalationWorker;
/** Build human-readable escalation message for doctor notification */
function buildEscalationMessage(patientName, doctorName, hospitalName, missedCount, chronicTag) {
    const tag = chronicTag ? ` [${chronicTag}]` : '';
    return (`${doctorName}, ${missedCount} appointment(s) missed consecutively${tag} by ${patientName} at ${hospitalName}. ` +
