  ...
  static async processQueue() {
    ...
    try {
      ...
      for (const alert of res.rows) {
        ...
        if (redis_1.default) {
          const acquired = await redis_1.default.eval(
            LUA_ACQUIRE,
            1,
            lockKey,
        ...
        try {
          ...
          if (redis_1.default) await redis_1.default.eval(LUA_RELEASE, 1, lockKey);
        }
      }
      await client.query('COMMIT');
    } catch (err) {
  ...
  static async attemptEscalatePrisma(alert) {
    ...
    try {
      ...
      if (sent) {
        ...
        await event_service_1.EventService.publish(
