...
(0, vitest_1.describe)('Phase 0A — canonical event envelope (acceptance criteria)', () => {
    (0, vitest_1.it)('1. legacy event with only eventType + payload is normalized with nulls', () => {
        const env = (0, outbox_1.normalizeLegacyOutbox)({ id: 'evt-1', eventType: 'FOO', payload: { a: 1 } });
    ...
    (0, vitest_1.it)('2. canonical event with all structured metadata is preserved', () => {
        ...
        (0, vitest_1.expect)(env.aggregate).toEqual({ aggregateType: 'ADMISSION', aggregateId: 'adm-9' });
        (0, vitest_1.expect)(env.scope).toEqual({ scopeType: 'HOSPITAL', hospitalId: 'hosp-1', tenantId: 'ten-1' });
        (0, vitest_1.expect)(env.actor).toEqual({ actorId: 'doc-1', actorType: 'DOCTOR', role: 'CLINICIAN' });
        (0, vitest_1.expect)(env.chain).toEqual({ correlationId: 'corr-1', causationId: 'caus-1', depth: 2 });
    ...
    (0, vitest_1.it)('6. hospital-scoped event', () => {
        ...
        (0, vitest_1.expect)(env.scope).toEqual({ scopeType: 'HOSPITAL', hospitalId: 'hosp-2', tenantId: null });
    });
    ...
    (0, vitest_1.it)('8. child event with correlationId + causationId', () => {
        ...
        (0, vitest_1.expect)(env.chain).toEqual({ correlationId: 'root', causationId: 'parent', depth: 0 });
    });
    (0, vitest_1.it)('9. depth zero root event', () => {
        const env = (0, outbox_1.normalizeLegacyOutbox)({ id: 'evt-9', eventType: 'ROOT', payload: {}, depth: 0 });
        (0, vitest_1.expect)(env.chain.depth).toBe(0);
    });
    (0, vitest_1.it)('10. existing DB row created before migration (no structured cols) processes', () => {
        const env = (0, outbox_1.normalizeLegacyOutbox)({
            id: 'evt-legacy',
            eventType: 'OLD',
    ...
    (0, vitest_1.it)('14. unknown legacy event does NOT receive fabricated identity/context', () => {
        const env = (0, outbox_1.normalizeLegacyOutbox)({ id: 'evt-14', eventType: 'MYSTERY', payload: {} });
        (0, vitest_1.expect)(env.scope.hospitalId).toBeNull();
