...
vi.mock('@haspataal/timeline', () => ({
  getTimelinePublisher: vi.fn().mockReturnValue({
    publishStandardEvent: vi.fn().mockResolvedValue(true),
  }),
}));
...
describe('Radiology Integration', () => {
  it('should run through the full lifecycle of a radiology order', async () => {
    ...
    expect(placeRes.id).toBe('order-123'); // success returns order now

    // 2. Accession Study
    // Mock the state to SCHEDULED
    prisma.imagingStudy.findUnique = vi.fn().mockResolvedValue({
      id: 'study-123',
      status: 'SCHEDULED',
      patientId: 'p1',
      hospitalId: 'h1',
