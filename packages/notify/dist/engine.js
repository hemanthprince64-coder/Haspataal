...
class NotificationEngine {
    ...
    async dispatchQueue(queueName, notificationId, priority) {
        let queue = this.queues.get(queueName);
        if (!queue) {
            queue = new bullmq_1.Queue(queueName, { connection });
            this.queues.set(queueName, queue);
        }
        await queue.add('notification', { notificationId }, {
            priority: this.getPriorityValue(priority),
            attempts: this.getMaxAttempts(priority),
            backoff: this.getBackoffStrategy(priority),
        });
    }
