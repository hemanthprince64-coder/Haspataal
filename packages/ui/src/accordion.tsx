...
>(({ className, children, ...props }, ref) => {
  ...
  const itemValue =
    (props as any).value || (props as any).parentElement?.getAttribute('data-value');
...
export const SimpleAccordion = ({ title, icon, isOpen, onToggle, children }: any) => {
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm transition-all duration-300">
      ...
      {isOpen && (
        <div className="p-5 animate-in fade-in slide-in-from-top-2 duration-300">{children}</div>
      )}
    </div>
  );
};
