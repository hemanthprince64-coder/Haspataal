        ...
        switch (action.type) {
            case 'create_timeline':
                ...
                if (!action.payload.title)
                    errors.push("Missing 'title' in create_timeline payload.");
                break;
            ...
            case 'update_record':
                if (!action.payload.table)
                    errors.push("Missing 'table' in update_record payload.");
                if (!action.payload.id)
                    errors.push("Missing 'id' in update_record payload.");
                if (!action.payload.data)
                    errors.push("Missing 'data' in update_record payload.");
                break;
            case 'call_api':
                if (!action.payload.url)
                    errors.push("Missing 'url' in call_api payload.");
                break;
            case 'assign_task':
                if (!action.payload.title)
                    errors.push("Missing 'title' in assign_task payload.");
                if (!action.payload.assignedTo)
                    errors.push("Missing 'assignedTo' in assign_task payload.");
                break;
            case 'escalate':
                if (!action.payload.reason)
                    errors.push("Missing 'reason' in escalate payload.");
                break;
