      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Patient: <span className="font-semibold">{order.patient.name}</span>
          </p>
        ...
        {order.items.map((item) => {
          ...
          return (
            <div key={item.id} className="border p-4 rounded bg-white shadow-sm">
              <div className="flex justify-between mb-4">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs uppercase ${
                    item.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100'
                  }`}
                >
                  Status: {item.status}
                </span>
              ...
              <form
                onSubmit={(e) => handleResultSubmit(e, item.id)}
              ...
              >
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Normal Range: {item.test.normalRangeText || 'N/A'}
                  </p>
                </div>
