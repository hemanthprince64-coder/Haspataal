    ...
    static async publish(eventType, payload, hospitalId, patientId = null, resourceId = 'global') {
        ...
        `, [hospitalId, patientId, eventType, JSON.stringify(payload), idempotencyKey]);
            ...
            if (redis_1.default) {
                ...
                await redis_1.default.xadd(streamKey, '*', 'event_id', eventRecord.id, 'hospital_id', eventRecord.hospital_id, 'payload', JSON.stringify(eventRecord));
            }
            else {
    ...
    static async subscribe(eventType, consumerGroup, consumerName, handler) {
        ...
        while (true) {
            ...
            try {
                ...
                const response = (await redis_1.default.xreadgroup('GROUP', consumerGroup, consumerName, 'COUNT', '10', 'BLOCK', '5000', 'STREAMS', streamKey, '>'));
                if (response) {
