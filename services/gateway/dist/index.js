...
const JWT_SECRET = new TextEncoder().encode(process.env.NEXTAUTH_SECRET || 'fallback_secret_for_dev_only');
const RATE_LIMITS = {
    PATIENT: 60,
    DOCTOR: 120,
    HOSPITAL_ADMIN: 200,
...
const requireRole = (...roles) => (req, res, next) => {
    if (!req.user || !roles.includes(req.user.role)) {
        req.log.warn({ requiredRoles: roles, actualRole: req.user?.role }, 'Insufficient permissions');
...
const requireHospitalTenant = (req, res, next) => {
    ...
    if (req.user?.hospitalId !== hospitalId) {
        req.log.warn({ requestedHospital: hospitalId, userHospital: req.user?.hospitalId }, 'Cross-tenant access denied');
        return res.status(403).json({
            success: false,
...
app.post('/v1/appointments', middleware_1.csrfProtection, requireAuth, rateLimiter, async (req, res) => {
...
app.patch('/v1/appointments/:id/status', middleware_1.csrfProtection, requireAuth, rateLimiter, async (req, res) => {
...
app.get('/v1/hospitals/:id/patients', requireAuth, requireRole('hospital_admin', 'HOSPITAL_ADMIN', 'super_admin', 'SUPER_ADMIN'), requireHospitalTenant, rateLimiter, async (req, res) => {
...
app.get('/v1/hospitals/:id/doctors', requireAuth, requireRole('hospital_admin', 'HOSPITAL_ADMIN', 'doctor', 'DOCTOR', 'super_admin', 'SUPER_ADMIN'), requireHospitalTenant, rateLimiter, async (req, res) => {
    try {
...
app.get('/v1/escalations', requireAuth, requireRole('doctor', 'DOCTOR', 'hospital_admin', 'HOSPITAL_ADMIN', 'super_admin', 'SUPER_ADMIN'), rateLimiter, async (req, res) => {
    try {
...
app.patch('/v1/escalations/:id/acknowledge', middleware_1.csrfProtection, requireAuth, requireRole('doctor', 'DOCTOR', 'hospital_admin', 'HOSPITAL_ADMIN', 'super_admin', 'SUPER_ADMIN'), rateLimiter, async (req, res) => {
