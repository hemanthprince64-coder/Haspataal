...
export class RetentionService {
  ...
  public static async processDischargeEvent(event: any) {
    ...
    if (summaryText.includes('delivery') || summaryText.includes('pregnancy')) {
      ...
      summaryText.includes('hypertension') ||
      summaryText.includes('ckd')
    ) {
      await this.scheduleChronicPathway(hospital_id, patient_id, doctor_id);
    } else {
      await this.scheduleGeneralPathway(
        hospital_id,
        patient_id,
        doctor_id,
        summaryText.includes('follow-up required'),
      );
    }
  }

  private static async schedulePregnancyPathway(
    hospitalId: string,
    patientId: string,
    doctorId: string,
  ) {
  ...
  private static async schedulePediatricsPathway(
    hospitalId: string,
    patientId: string,
    doctorId: string,
  ) {
  ...
  private static async scheduleChronicPathway(
    hospitalId: string,
    patientId: string,
    doctorId: string,
  ) {
  ...
  private static async scheduleGeneralPathway(
    hospitalId: string,
    patientId: string,
    doctorId: string,
    needs30Day: boolean,
  ...
  private static async insertFollowUps(
    hospitalId: string,
    patientId: string,
    doctorId: string,
    pathway: string,
