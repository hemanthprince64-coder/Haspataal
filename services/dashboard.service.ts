...
export async function getDashboardMetrics(hospitalId: string): Promise<DashboardMetrics> {
  ...
  const occupiedBeds = bedStats.find((g) => g.status === 'OCCUPIED')?._count.status ?? 0;
  const occupancyPct = totalBeds > 0 ? Math.round((occupiedBeds / totalBeds) * 100) : 0;

  // ── Process appointment stats ──────────────────────────────
  const apptTotal = todayApptStats.reduce((sum, g) => sum + g._count.status, 0);
  const apptCompleted = todayApptStats.find((g) => g.status === 'COMPLETED')?._count.status ?? 0;
  const apptPending = todayApptStats.find((g) => g.status === 'BOOKED')?._count.status ?? 0; // The existing status is BOOKED/CONFIRMED
