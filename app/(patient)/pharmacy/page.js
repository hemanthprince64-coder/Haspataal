  ...
  return (
    <main className="container max-w-5xl mx-auto px-6 py-10 animate-fade-in text-slate-900">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Superfast 2-hour delivery to your doorstep.
          </p>
        </div>
        <div className="flex items-center gap-4 bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
          <Clock className="w-5 h-5 text-blue-600" />
          <div className="text-sm font-black text-slate-700">
            Delivery in <span className="text-blue-600">45 Mins</span>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-12">
        ...
        <Button
          size="lg"
          className="h-16 px-8 bg-slate-900 border-2 border-slate-900 hover:bg-slate-800 text-white rounded-2xl font-black text-lg shadow-xl shadow-slate-900/20 transition-all active:scale-95"
        >
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-6 scrollbar-none mb-4">
        {categories.map((cat, i) => (
          <Badge
            ...
            className={`h-11 px-6 rounded-xl font-black text-xs uppercase tracking-widest cursor-pointer transition-all ${i === 0 ? 'bg-blue-600 hover:bg-blue-700' : 'border-slate-200 text-slate-500 hover:border-blue-300 hover:bg-blue-50'}`}
          >
            {cat}
          </Badge>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {medicines.map((med) => (
          <Card
            key={med.id}
            className="group rounded-[2rem] border-slate-200/60 shadow-xl shadow-slate-200/5 bg-white overflow-hidden hover:border-blue-300 hover:shadow-2xl transition-all duration-500"
          >
            ...
            <CardContent className="p-6">
              <div className="mb-4">
                <h4 className="font-black text-lg tracking-tight line-clamp-1 group-hover:text-blue-600 transition-colors uppercase">
                  {med.name}
                </h4>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <div className="text-[10px] font-bold text-slate-400 line-through">
                    MRP ₹{Math.round(med.price * 1.15)}
                  </div>
                </div>
                <Button
                  size="icon"
                  onClick={() => setCartCount((c) => c + 1)}
                  className="h-12 w-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 active:scale-90 transition-all"
                >
      ...
      <Card className="rounded-[2.5rem] bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-700 border-none p-1 flex items-center overflow-hidden shadow-2xl shadow-blue-500/20 group">
        <CardContent className="w-full bg-white/95 backdrop-blur-sm rounded-[2.35rem] p-8 md:p-12 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
          ...
          <div className="flex-1 text-center md:text-left">
            <Badge
              variant="outline"
              className="mb-4 border-blue-200 text-blue-600 font-black px-4 py-1 text-[10px] uppercase tracking-widest"
            >
              Prescription Orders
            </Badge>
            ...
            <p className="text-slate-500 text-lg font-medium tracking-tight mb-0">
              Don&apos;t have time to search? Simply upload your prescription and our experts will
              handle everything.
            </p>
          </div>
          ...
          <Button
            size="lg"
            className="h-16 px-10 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-black text-lg shadow-xl shadow-slate-900/20 flex-shrink-0 transition-all active:scale-95"
          >
