    ...
    <main className="container max-w-4xl mx-auto px-6 py-10 animate-fade-in text-slate-900">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Badge
              variant="secondary"
              className="text-indigo-700 bg-indigo-100/50 hover:bg-indigo-100 border-indigo-200 px-3 py-1 font-bold text-[10px] uppercase tracking-[0.2em] backdrop-blur-sm"
            >
          ...
          <p className="text-slate-500 text-lg font-medium tracking-tight">
            Store policy details for cashless admissions and rapid claims.
          </p>
        </div>
        <Button
        ...
        >
          {showForm ? (
            ...
            <>
              <Plus className="w-6 h-6 mr-3" /> Add Policy
            </>
          )}
        </Button>
      </div>
        ...
        <CardContent className="w-full bg-white/95 backdrop-blur-sm rounded-[1.85rem] p-6 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="flex-1">
            <p className="text-slate-600 font-bold leading-relaxed">
              <span className="text-indigo-600 font-black uppercase tracking-widest text-[10px] block mb-1">
                Financial Safety
              </span>
              Keeping your insurance updated ensures zero-delay processing during hospital
              admissions and emergency scenarios.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="mb-12 rounded-[2.5rem] border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in zoom-in-95 duration-500">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            ...
            <CardDescription className="text-slate-500 font-medium tracking-tight">
              Enter details exactly as printed on your insurance ID card.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <form action={formAction} className="space-y-8">
              ...
              >
                {isPending ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" /> Virtualizing Policy...
                  </>
                ) : (
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          ...
          <Badge
            variant="outline"
            className="border-slate-200 text-slate-400 font-bold tracking-tight px-3"
          >
            ...
            {insuranceList.length === 0 ? (
              <div className="py-24 text-center bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200/60 animate-in fade-in zoom-in duration-500">
                ...
                <p className="text-slate-500 text-lg font-medium max-w-sm mx-auto leading-relaxed">
                  Store your insurance details now to avoid last-minute documentation during
                  emergencies.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {insuranceList.map((policy) => (
                  <Card
                    key={policy.id}
                    className="group rounded-[2.5rem] bg-gradient-to-br from-white to-slate-50/50 border-slate-200/60 shadow-xl shadow-slate-200/5 overflow-hidden hover:border-indigo-300 hover:shadow-2xl transition-all duration-500 relative"
                  >
                    ...
                    <CardContent className="p-8 relative z-10">
                      <div className="flex justify-between items-start gap-4 mb-8">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-black text-2xl tracking-tight leading-tight group-hover:text-indigo-600 transition-colors uppercase">
                              {policy.company}
                            </h3>
                      ...
                      <div className="space-y-4 mb-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50">
                            ...
                            <div className="text-base font-black text-slate-700 tracking-tight">
                              {policy.expiryDate
                                ? new Date(policy.expiryDate).toLocaleDateString(undefined, {
                                    month: 'short',
                                    year: 'numeric',
                                  })
                                : 'Lifetime'}
                      ...
                      <Button
                        variant="outline"
                        className="w-full border-slate-200 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] group-hover:bg-indigo-50 group-hover:border-indigo-200 transition-all"
                      >
